...
def main():
    ...
    if comm.rank != 0:
        ...
        while n_recieved < n_sims:
            Xw, Yw, pos, param, x, y = comm.recv(source = MPI.ANY_SOURCE)
            ...
            while len(X) >= batch_size:
                ...
                ofile.create_dataset('{0}/positions'.format(counter), data=np.array(positions[-batch_size:]), compression='gzip')
                ofile.create_dataset('{0}/x_windows'.format(counter), data=add_channel(np.array(X_windows[-batch_size:], dtype=np.uint8)), compression = 'lzf')
                ofile.create_dataset('{0}/y_windows'.format(counter),
                                     data=add_channel(np.array(Y_windows[-batch_size:], dtype=np.uint8)),
                                     compression='lzf')
