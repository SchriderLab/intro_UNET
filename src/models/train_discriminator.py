...
def train_cnn_model(model, configFile, weightFileName, training_generator, validation_generator, gpus, tf_logdir):
    ...
    tbCallBack = TrainValTensorBoard(log_dir=tf_logdir, histogram_freq=0, write_graph=True, write_images=True,
                                     write_grads=False, update_freq='epoch')

    rlrop = keras.callbacks.ReduceLROnPlateau(monitor='val_loss', factor=LROnPlateau_factor,
                                              patience=LROnPlateau_patience, verbose=1, mode='auto', min_delta=0.0001,
                                              cooldown=0, min_lr=0)

    earlystop = keras.callbacks.EarlyStopping(monitor='val_loss', min_delta=0, patience=EarlyStoppingPatience,
                                              verbose=0, mode='auto')
...
def parse_args():
    ...
    parser.add_argument("--train_config", default="training_configs/adam_default",
                        help="contains the settings for training, the optimizer, learning rate, etc.")
...
def main():
    ...
    history = train_cnn_model(model, args.train_config, weightFileName, training_generator, validation_generator, gpus,
                              tf_logdir)
